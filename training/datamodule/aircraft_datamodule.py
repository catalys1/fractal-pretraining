...
__all__ = ['AircraftDataModule']


class AircraftDataModule(LightningDataModule):
    def __init__(
        self,
        data_dir: str,
        batch_size: int = 64,
    ...
    def train_dataloader(self):
        if self.data_train is None:
            tform = transforms.Compose([
                transforms.RandomResizedCrop(self.dims[1:], scale=(0.25, 1.0)),
                transforms.RandomHorizontalFlip(0.5),
                transforms.ColorJitter(0.25, 0.25, 0.25, 0.1),
                to_tensor(self.normalize),
            ])
            self.data_train = Aircraft(
